...
class Tracker:
    ...
    def _build_kernel(self, save_source_as):
        ...
        if self._local_particle_mode == LocalParticleVar.SHARED_COPY and isinstance(
            ...
            src_lines[-1] += "extern /*gpusharedmem*/ char local_fields[];\r\n"
            use_shared_copy = True
        ...
        if use_shared_copy and isinstance(context, xo.ContextPyopencl):
            kernel_args.append(
                xo.LocalMemoryPyopenclArg(
                    xo.Int8,
                    name="local_fields",
                    locmem_per_work_group=16,
                    locmem_per_work_item=136,
                )
            )
        ...
        if use_shared_copy and isinstance(context, xo.ContextPyopencl):
            ...
            pdb.set_trace()
            k = context.kernels.track_line
            kernel_args[-1].update(k)
            assert kernel_args[-1].is_assigned_to(k)
        elif use_shared_copy and isinstance(context, xo.ContextCupy):
            k = context.kernels.track_line
            shmem_size = k.calc_shared_mem_size(
                k.block_size, bytes_per_thread=136, bytes_per_block=8
            )
            k.shared_mem_size = shmem_size
